...
/**
 * Recreates the AirPods Pro listening-mode glyphs used by Apple's control UI.
 * Noise cancelling uses the ring glyph, transparency the dotted glyph and the
 * off state reuses the adaptive (person with sparkles) glyph, so all three
 * states stay visually distinct at volume-panel size.
 */
final class AirPodsModeDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        float centerY = mode == AncController.MODE_TRANSPARENCY ? 35.5329f : 34.2104f;
        float bodyTop = mode == AncController.MODE_TRANSPARENCY ? 55.1158f : 53.7933f;
        float bodyBottom = mode == AncController.MODE_TRANSPARENCY ? 76.6575f : 75.3350f;
        ...
        if (mode == AncController.MODE_TRANSPARENCY) {
            ...
            drawNoiseControlArc(canvas, 1.0f);
        }
        canvas.restoreToCount(save);
    }

    /**
     * Four-pointed sparkle with concave edges, matching the adaptive listening-mode glyph.
     */
    ...
    private void drawNoiseControlArc(Canvas canvas, float opacity) {
        Path arc = new Path();
        ...
        paint.setAlpha(scaledAlpha(opacity));
        canvas.drawPath(arc, paint);
    }

    private int scaledAlpha(float opacity) {
        return Math.round(alpha * opacity);
    }
    ...
    public void setTintMode(PorterDuff.Mode tintMode) {
        // ImageView supplies a tint list; the glyph is drawn directly in that color.
    }
